...
export function checkedOutputPath(target, allowedDirs, label = "screenshot") {
  ...
  const dirs = [
    ...allowedDirs,
    // Sandboxes that don't use the canonical /workspace root can point the
    // guard at their real project root via WORKSPACE_ROOT.
    ...(process.env.WORKSPACE_ROOT ? [process.env.WORKSPACE_ROOT] : []),
  ];
